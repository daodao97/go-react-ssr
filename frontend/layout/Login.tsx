...
import { getWebsite } from '../lib/i18n';
        ...
        <Dialog.Root open={open} onOpenChange={onOpenChange}>
            <Dialog.Portal>
                ...
                <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg p-6 shadow-lg max-w-md w-full data-[state=open]:animate-contentShow">
                    <Dialog.Title className="text-xl font-bold mb-4">登录</Dialog.Title>
                    ...
                    <div className="space-y-4">
                        {authProviders.map((provider) => {
                            if (provider.Provider === 'github') {
                                ...
                                return <GoogleLogin key={provider.Provider} data={provider} />;
                            } else {
                                return (
                                    <button
...
// google登录组件
function GoogleLogin({ data }: { data: any }) {
    useEffect(() => {
        // 加载 Google 登录脚本
